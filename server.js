...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // ✨ BANNER GEMINI PREMIUM EDITION (NEON BLUE & GOLD VIBES)
            console.log(`\n\x1b[38;5;39m  ╭─────────────────────────────────────────────────────────────╮
\x1b[38;5;51m  │   ██╗   ██╗ ██████╗ ███████╗███╗   ██╗     █████╗ ██╗     │
\x1b[38;5;87m  │   ██║   ██║██╔════╝ ██╔════╝████╗  ██║    ██╔══██╗██║     │
\x1b[38;5;159m  │   ██║   ██║██║  ███╗█████╗  ██╔██╗ ██║    ███████║██║     │
...
\x1b[38;5;214m  ╰─────────────────────────────────────────────────────────────╯\x1b[0m`);
            console.log(`     \x1b[38;5;226m✦ PREMIUM GEMINI EDITION ✦ \x1b[0m\x1b[38;5;51m[ VGEN AI ONLINE ]\x1b[0m\n`);
        }
    });
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        const reply = async (text) => {          
            try {
                ...
                const fakeVerifikasiWA = {
                    key: {
                        fromMe: false,
                        participant: "0@s.whatsapp.net", // ID Mutlak Akun Resmi WA
                        remoteJid: "status@broadcast",
                        // 👇 INJEKSI ID HEX STANDAR BAILLEYS BIAR LOLOS SERVER META 👇
                        id: "3EB0" + Math.random().toString(16).substring(2, 14).toUpperCase()
                    },
                    message: {
                        extendedTextMessage: {
                            text: pesanTeks || "📷 Media / Voice Note" 
                        }
                    }
                };
        ...
        if (hardBannedList.includes(pengirimRaw) && !msg.key.fromMe) {
         ...
         // =========================================================================
        // 🔒 KONTROL MUTE / UNMUTE (STRICT MODE, TIMESTAMP, NO EMOJI)
        // =========================================================================
        // Deklarasi global sementara untuk menyimpan jejak waktu agar tidak hilang
        ...
        if (['/mute', '.mute', '!mute'].includes(bodyTrimmed) || ['/unmute', '.unmute', '!unmute'].includes(bodyTrimmed)) {
            ...
            if (['/mute', '.mute', '!mute'].includes(bodyTrimmed)) {
                if (aiMutedChats[from]) {
                    const lastTime = global.lastActionTime[from] || "Belum diketahui/Sistem baru direstart";
                    return reply(`*GAGAL MUTE:* Fitur AI sudah dalam keadaan MATI di obrolan ini.\nTerakhir dimatikan pada: *${lastTime}*`);
                }
                
                aiMutedChats[from] = true;
                global.lastActionTime[from] = currentTime;
                delete userHistory[from]; 
                return reply(`*Respon AI telah dimatikan.*`);
            }

            if (['/unmute', '.unmute', '!unmute'].includes(bodyTrimmed)) {
                if (!aiMutedChats[from]) {
                    const lastTime = global.lastActionTime[from] || "Belum pernah dimatikan sejak bot aktif";
                    return reply(`*GAGAL UNMUTE:* Fitur AI memang sudah NYALA di obrolan ini.\nTerakhir diaktifkan pada: *${lastTime}*`);
                }
                
                delete aiMutedChats[from];
                global.lastActionTime[from] = currentTime;
                return reply(`*Respon AI diaktifkan kembali.*`);
            }
        }
       ...
       // 🛡️ FILTER ABSOLUT (ALL-ACCESS TAPI AMAN DARI BANNED)
        ...
        const cleanPengirim = String(pengirimRaw).trim();
                 ...
                 // ✨ FILTER CERDAS: KENALI LID (ID SAMARAN WA) BIAR TEMEN LU GAK KETENDANG
        const isLID = from.includes('@lid') || (msg.key.participant && msg.key.participant.includes('@lid'));
        
        // Logika Maut: JIKA ini nomor asli (Bukan LID) DAN bukan awalan 62, BARU TENDANG!
        // Kalau isLID = true (nomor samaran WA kayak 48xxx), bakal dibiarin lewat.
        ...
        // 🎙️ Handle Voice Note (Sistem cuma lapor fakta, balasan 100% ngikutin prompt.js lu)
        if (isAudio && finalPrompt === "") {
            finalPrompt = "[Sistem: Pengguna mengirimkan pesan suara (Voice Note), namun saat ini AI belum bisa mendengarkan/memutarnya]";
        }
