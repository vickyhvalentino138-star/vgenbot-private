...
\x1b[38;5;213m  ╰─────────────────────────────────────────────────────────────╯\x1b[0m`);
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // =========================================================================
        // 🔒 KONTROL MUTE / UNMUTE (STRICT MODE & SUPPORT TARGET NOMOR)
        // =========================================================================
        global.lastActionTime = global.lastActionTime || {};

        const isMuteCmd = bodyTrimmed.startsWith('/mute') || bodyTrimmed.startsWith('.mute') || bodyTrimmed.startsWith('!mute');
        const isUnmuteCmd = bodyTrimmed.startsWith('/unmute') || bodyTrimmed.startsWith('.unmute') || bodyTrimmed.startsWith('!unmute');
        ...
        if (isMuteCmd || isUnmuteCmd) {
            ...
            if (isMuteCmd) {
                if (aiMutedChats[targetMute]) {
                    ...
                    return reply(`*GAGAL MUTE:* AI sudah NONAKTIF untuk target ini.\nTerakhir: *${lastTime}*`);
                }
                
                aiMutedChats[targetMute] = true;
                global.lastActionTime[targetMute] = currentTime;
                delete userHistory[targetMute]; 
                return reply(`*Respon AI telah dimatikan untuk ${targetMute === from ? 'obrolan ini' : targetMute}.*`);
            }

            if (isUnmuteCmd) {
                if (!aiMutedChats[targetMute]) {
                    const lastTime = global.lastActionTime[targetMute] || "Belum pernah dimatikan";
                    return reply(`*GAGAL UNMUTE:* AI memang sudah AKTIF untuk target ini.\nTerakhir: *${lastTime}*`);
                }
                
                delete aiMutedChats[targetMute];
                global.lastActionTime[targetMute] = currentTime;
                return reply(`*Respon AI diaktifkan kembali untuk ${targetMute === from ? 'obrolan ini' : targetMute}.*`);
            }
        }
            ...
            try {
                ...
                let targetMsg = hasMedia ? realMsg[type] : (quotedMediaMsg ? quotedMediaMsg[Object.keys(quotedMediaMsg)[0]] : null);
                 ...
                 if (activeProvider === 'OPENAI') {
                    ...
                    // 🕵️ DETEKSI BLOK JSON RAHASIA DARI AI
                    const btnRegex = /===BUTTON_DATA===([\s\S]*?)===END_BUTTON_DATA===/;
                    const match = rawResponse.match(btnRegex);
                    if (match) {
                        try {
                            buttonPayload = JSON.parse(match[1].trim());
                            // Hapus block JSON dari teks biar ga diliat lawan bicara!
                            rawResponse = rawResponse.replace(btnRegex, '').trim(); 
                        } catch(e) { console.log("\x1b[38;5;196m[ERROR]\x1b[0m Gagal parsing JSON Button dari AI."); }
                    }
                    ...
                    // 🚀 EKSEKUSI PEMANGGILAN BUTTON ATAU PESAN NORMAL
                    if (buttonPayload && (buttonPayload.type === 'copy' || buttonPayload.type === 'game')) {
                        const { generateWAMessageFromContent, proto } = require('@whiskeysockets/baileys');
                        let buttons = [];
                        
                        // RAKIT TOMBOL SALIN & LINK
                        if (buttonPayload.type === 'copy' && buttonPayload.copy_data) {
                            buttons.push({
                                name: "cta_copy",
                                buttonParamsJson: JSON.stringify({
                                    display_text: buttonPayload.copy_data.copy_btn_title || "Salin Teks",
                                    id: "copy_" + Date.now(),
                                    copy_code: buttonPayload.copy_data.text_to_copy
                                })
                            });
                            if (buttonPayload.copy_data.url) {
                                buttons.push({
                                    name: "cta_url",
                                    buttonParamsJson: JSON.stringify({
                                        display_text: buttonPayload.copy_data.url_btn_title || "Buka Link",
                                        url: buttonPayload.copy_data.url,
                                        merchant_url: buttonPayload.copy_data.url
                                    })
                                });
                            }
                        } 
                        // RAKIT TOMBOL GAME/PILIHAN INTERAKTIF (MAX 10)
                        else if (buttonPayload.type === 'game' && buttonPayload.game_data && buttonPayload.game_data.choices) {
                            buttonPayload.game_data.choices.slice(0, 10).forEach((choice, idx) => {
                                buttons.push({
                                    name: "quick_reply",
                                    buttonParamsJson: JSON.stringify({
                                        display_text: choice,
                        ...
                        // JAHIT KE NATIVE FLOW MESSAGE BAILEYS                                             
                        const fakeVerifikasiWA_Button = {
                            key: {
                                fromMe: false,
                        ...
                        // JAHIT KE NATIVE FLOW MESSAGE BAILEYS + INJEK CENTANG BIRU
                        let msgNode = generateWAMessageFromContent(from, {
                            viewOnceMessage: {
                                message: {
                                    messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                                    interactiveMessage: proto.Message.InteractiveMessage.create({
                                        body: proto.Message.InteractiveMessage.Body.create({ text: rawResponse }),
                                        footer: proto.Message.InteractiveMessage.Footer.create({ text: " 亗 VGen AI Ultimate" }),
                        ...
                        }, { userJid: sock.user.id, quoted: fakeVerifikasiWA_Button }); // 🚀 INJEKSI MUTLAK DISINI!                     
                        await sock.relayMessage(from, msgNode.message, { messageId: msgNode.key.id });          
            
                        console.log(`\x1b[38;5;82m[DISPATCH SUCCESS]\x1b[0m Pesan INTERAKTIF (Button) meluncur ke: ${from.split('@')[0]}`);
                    } else {
                        // Kalo AI ga ngeluarin format button, kirim pake reply centang biru biasa!
                        await reply(rawResponse);                    
                    }

                    // 🔥 KOTAK TUMPUL PREMIUM LOG BALASAN AI
                    const responPotong = aiResponse.replace(/\n/g, ' ').length > 50 ? aiResponse.replace(/\n/g, ' ').substring(0, 50) + '...' : aiResponse.replace(/\n/g, ' ');
