...
\x1b[38;5;213m  ╰─────────────────────────────────────────────────────────────╯\x1b[0m`);
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (isMuteCmd || isUnmuteCmd) {
            ...
            // 🎯 DETEKSI TARGET (Apakah mute obrolan ini, atau mute nomor lain?)
            let targetMute = from; // Default: Obrolan tempat lu ngetik
            const splitCmd = bodyTrimmed.split(' ');
            
            if (splitCmd.length > 1) {
                // Kalo lu ngetik: .mute 628xxx (Bersihin spasi & karakter aneh)
            ...
            if (isMuteCmd) {
                if (aiMutedChats[targetMute]) {
                    ...
                    return reply(`*GAGAL MUTE:* AI sudah MATI untuk target ini.\nTerakhir: *${lastTime}*`);
                }
            ...
            if (isUnmuteCmd) {
                if (!aiMutedChats[targetMute]) {
                    ...
                    return reply(`*GAGAL UNMUTE:* AI memang sudah NYALA untuk target ini.\nTerakhir: *${lastTime}*`);
                }
